...
class GetSearchBookWithKeywordPagingSource(
    ...
    override fun getRefreshKey(state: PagingState<Int, ResSearchBook.ResponseData.BookWrapper>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ResSearchBook.ResponseData.BookWrapper> {
        // LoadParams : 로드할 키와 항목 수 , LoadResult : 로드 작업의 결과
        return try {
            // 키 값이 없을 경우 기본값을 사용함
            val position = params.key ?: STARTING_PAGE_INDEX

            val response = apiService.getSearchBookWithKeyword(
                authKey = authToken,
                format = format,
                pageNo = position,
                pageSize = 10,
            ...
            post?.let {
                ...
                LoadResult.Page(
                    ...
                    nextKey = if (position == post.response.numFound / post.response.request.pageSize + 1) null else position + 1,
                    itemsBefore = 0,
                    itemsAfter = 0
                )
